...
2.4以上版本中青阅读看看赚阅读，本脚本改自@sunert大佬，新手制作，如有问题请见谅！其他功能后续补充~
在boxjs中自行手动改写body，添加的请求体越多，获得青豆次数越多，本脚本不包含任何推送通知。
食用方法看README.md
//抓取到的body是zqkan--=9XXX形式,请手动改成p=9XXX再填入boxjs中。
//多个请求体时用'&'号隔开" ‼️
//需手动填入ck、阅读body、看看赚body，目前仅支持自动阅读。
*/
...
let bodys = $.getdata("zqbody_body1");
...
let indexLast = $.getdata('zqbody_body_index1');
...
function AutoRead() {
  ...
  return new Promise((resolve, reject) => {
    ...
    $.post(url, async (error, response, data) => {
      ...
      $.setdata(res+"", 'zqbody_body_index1');
